...
<body>
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<tr>
                            ...
                            <td>" . htmlspecialchars($row['fines']) . "</td> <!-- Display fines -->
                            <td>
                                <button class='status-btn status-btn-{$statusClass}' onclick='changeStatus({$row['member_id']}, \"" . ($row['status'] === 'enabled' ? 'disabled' : 'enabled') . "\")'>{$statusText}</button>
                            </td>
                        </tr>";
                    }
                }
    ...
    <script>
        ...
        function editFines(memberId) {
            let fines = prompt("Enter the new fines amount:");

            if (fines != null) {
                if (!isNaN(fines) && fines >= 0) {
