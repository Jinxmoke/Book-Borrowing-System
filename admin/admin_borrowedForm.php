...
// Fetch borrowed books with search, filters, and pagination
$sql = "SELECT b.*, u.email, u.member_id
        ...
        ($book_type !== 'all' ? "AND b.book_type = ? " : "") . 
        "LIMIT ?, ?";
$stmt = $conn->prepare($sql);

$searchTerm = "%$search%";
if ($book_type !== 'all') {
    $stmt->bind_param("ssii", $searchTerm, $book_type, $offset, $limit);
} else {
...
$sqlTotal = "SELECT COUNT(*) AS total
             ...
             ($book_type !== 'all' ? "AND book_type = ? " : "");
$stmtTotal = $conn->prepare($sqlTotal);

if ($book_type !== 'all') {
    $stmtTotal->bind_param("ss", $searchTerm, $book_type);
} else {
    ...
    <div class="container">
        ...
        <form method="GET" class="search-and-filter">
            ...
            <div class="filter-bar">
                ...
                <select id="book_type" name="book_type" onchange="this.form.submit()">
                    <option value="all" <?php echo $book_type === 'all' ? 'selected' : ''; ?>>All Types</option>
                    <option value="physical" <?php echo $book_type === 'physical' ? 'selected' : ''; ?>>Physical Books</option>
                    <option value="ebook" <?php echo $book_type === 'ebook' ? 'selected' : ''; ?>>E-Books</option>
                </select>
            </div>
        </form>
        
<button class="updatebook" onclick="handleOverdueBooks()">Update E-Books Status</button>
...
<div id="bookContainer" class="book-grid">
    <?php while ($row = $result->fetch_assoc()): ?>
        <div class="book-card">
            ...
            <div class="book-info">
                ...
                <p><strong>Book Type:</strong> <?php echo ucfirst(htmlspecialchars($row['book_type'])); ?></p>
            ...
            <div class="book-actions">
                <?php if ($row['due_date'] < date('Y-m-d') && $row['book_type'] !== 'ebook'): ?>
                ...
                <?php if ($row['book_type'] === 'ebook' || ($row['due_date'] >= date('Y-m-d') && $row['book_type'] !== 'ebook')): ?>
    ...
    <?php endwhile; ?>
        ...
        <div class="pagination">
            <?php if ($page > 1): ?>
                <a href="?page=<?php echo $page - 1; ?>&search=<?php echo urlencode($search); ?>&filter=<?php echo $filter; ?>&book_type=<?php echo $book_type; ?>">Previous</a>
            <?php endif; ?>
            <?php if ($page < $totalPages): ?>
                <a href="?page=<?php echo $page + 1; ?>&search=<?php echo urlencode($search); ?>&filter=<?php echo $filter; ?>&book_type=<?php echo $book_type; ?>">Next</a>
            <?php endif; ?>
        </div>
    </div>
    ...
    <script>
        ...
        <script>
        function handleOverdueBooks() {
            fetch('remove_overdue_books.php', {
                method: 'POST',
            })
            .then(response => response.text())
            .then(data => {
                alert(data); // Show the result in an alert
            })
            .catch(error => {
                console.error('Error:', error);
                alert('An error occurred while processing overdue books.');
            });
        }
    </script>
</body>
