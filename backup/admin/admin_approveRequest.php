...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'approve') {
    ...
    // If the user already has the book, don't insert it again
    if ($check_result->num_rows > 0) {
      
        // Book is already borrowed by the user, handle accordingly
        header("Location: admin_returnedBooksForm.php?message=This book is already borrowed by the user.");
        exit();
    }
    ...
    if ($book_type == 'ebook') {
        ...
        // Update the book's status in the manage_books table if the due date has passed
    ...
    header("Location: admin_pendingRequest.php?message=Request approved, book borrowed, and request removed.");
    exit();
}
