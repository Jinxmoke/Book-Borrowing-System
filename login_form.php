...
if (isset($_POST['submit'])) {
    ...
    // Query to check user details along with fines
    $select = "SELECT * FROM user_info WHERE email = ? AND status = 'enabled'";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        // Hash the password using md5 before comparing
        ...
        if ($hashed_pass === $row['password']) {
            ...
            $fines = (int)$row['fines']; // Fetch the fines count for the user

            if ($email_verified !== 1) {
                $error[] = 'Please verify your email before logging in.';
            } elseif ($fines >= 3) {
                // If the user has 3 or more fines, show an alert and disable login
                $error[] = 'Your account has accumulated 3 fines, and as a result, it has been disabled.';
            } else {
                ...
                if ($row['role'] == 'admin') {
                    ...
                    header('Location: user/user_catalogForm');
                    exit();
                } elseif ($row['role'] == 'lender') {
                    header('Location: lender/ldr_dashboardForm');
                    exit();
                }
            }
        } else {
...
<div class="login-form-container">
    <form action="" method="post">
        ...
        <p class="login-p">Don't have an account? <a href="register_form">Register</a></p>
    </form>
</div>
